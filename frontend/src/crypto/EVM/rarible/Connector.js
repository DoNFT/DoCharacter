        ...
        this._RaribleConnector.connection.subscribe(async (con) => {
            ...
            if(con.status === ConnectionSteps.connected && con.connection){
                ...
                if(
                    ...
                    console.warn('Skip repeated connected event')
                    return
                }

                try{
                    const isSwitched = await this.switchNetwork(connection.wallet.ethereum.config.web3.currentProvider, this._connectToNetworkId)
